...
export const useIssuesListQuery = () => {
  const favoriteList = useAppSelector((state: RootState) => state.favoriteList);

  const queries = useQueries(favoriteList.map((row, _index, array) => {
    return (
      {queryKey: issuesKeys.list(array.length, row.id), queryFn: () => getIssuesListAPI(row.full_name, 1)}
    )
  }));

  const isLoading = !!queries.length && queries.every(query => query.isFetching);
  const items = queries.length ? queries.map(query => query.data).flat() : []
  const total_count = items.length || 0;

  return { isLoading, items, total_count };
};
